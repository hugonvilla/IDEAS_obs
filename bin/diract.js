...
const INSTANCE_ID = [ 0x00, 0x00, 0xbc, 0xda];
...
const PROXIMITY_RSSI_THRESHOLD = -80;
...
const BROADCAST_PERIOD_MILLISECONDS = 1800;
...
const ENABLE_ACCELEROMETER = true; // Enable on Puck.js v2 only!
const ENABLE_BUTTON = true;
const BLINK_ON_PROXIMITY = false;
const BLINK_ON_DISTANCING = false;
const BLINK_ON_DIGEST = false;
...
function toggleSleepWake() {
  ...
  if(isSleeping) {
    ...
    if(!fullyAwake && button_press_counter === 5) {
      fifthPressTime = new Date();
      console.log('here4');
    }
    if(fullyAwake && button_press_counter === 10){
      tenthPressTime = new Date();
      console.log('here5');
    }
    var nowElapsed = new Date() - firstPressTime;
    if(button_press_counter >=0 && nowElapsed >= 5000){
      button_press_counter =1;
      firstPressTime = new Date();
      console.log('here6');
    }

    var timeElapsed5 = fifthPressTime - firstPressTime;
    if(!fullyAwake && button_press_counter >= 5 && timeElapsed5 <= 5000 ) {
        fullyAwake = true;
    ...
    var timeElapsed10 = tenthPressTime - firstPressTime;
    if(fullyAwake && button_press_counter >= 10 && timeElapsed10 <= 5000 ) {
        blink(LED1, LED_BLINK_MILLISECONDS); // Red = sleep
        initiateSleep = true;
        fullyAwake = false;
        button_press_counter = 0;
